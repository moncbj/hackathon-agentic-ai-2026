...
/** Validates the text pasted into the "Help me with my CV" panel. Returns an error message or null. */
export function validateCvText(text: string): string | null {
    const trimmed = text.trim();
    if (trimmed.length < LIMITS.cvTextMin) return 'Paste a few lines from your CV, portfolio or projects first.';
    if (trimmed.length > LIMITS.cvTextMax) {
        return `Use at most ${LIMITS.cvTextMax.toLocaleString('en-US')} characters. Paste only the most relevant parts.`;
    }
    return null;
}

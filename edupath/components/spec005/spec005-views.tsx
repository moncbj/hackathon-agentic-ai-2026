...
export function TutorView() {
  const welcomeMessage: Message = { id: 1, author: "tutor", text: "Hi Alex! I can help you understand your plan, practice a concept, or use one of your notebooks." };
  ...
  function newChat() { setMessages([{ ...welcomeMessage, id: Date.now() }]); setQuestion(""); setFocusSkill(""); setSelectedNotebookIds([]); setTyping(false); }

  return <div className="mx-auto max-w-6xl space-y-5">
    <div className="flex items-end justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-wider text-violet-600">AI tutor</p><h1 className="mt-1 text-3xl font-black">Learn by asking</h1></div><Button onClick={newChat} className="bg-violet-600 hover:bg-violet-700">+ New chat</Button></div>
    <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_17rem]"><Card className="overflow-hidden"><div className="border-b border-slate-100 bg-violet-50 px-5 py-4"><p className="font-bold text-violet-950">Ari · your learning tutor</p><p className="text-sm text-violet-800">Friendly, contextual help for your path.</p></div>
    ...
    </Card><aside className="space-y-4"><Card><CardContent><p className="text-xs font-black uppercase tracking-wider text-violet-600">Learning workspace</p><h2 className="mt-2 font-black">Your context</h2><div className="mt-4 space-y-3">{skills.slice(0, 2).map((skill) => <div key={skill.id} className="rounded-lg bg-slate-50 p-3"><p className="text-sm font-bold">{skill.name}</p><p className="text-xs text-slate-500">Level {skill.level} · {skill.status}</p></div>)}</div></CardContent></Card><Card><CardContent><p className="text-xs font-black text-amber-700">NOTES IN CONTEXT</p><p className="mt-2 text-sm text-slate-600">Select a notebook above to make its ideas available in this conversation.</p></CardContent></Card></aside></div>
  </div>;
}
...
export function ProgressView() {
  ...
  return <div className="mx-auto max-w-6xl space-y-6">
    ...
    <section className="grid gap-4 sm:grid-cols-4">{[["Skills acquired", "24", "text-emerald-600"], ["Current mastery", "78%", "text-violet-600"], ["Learning time", "6h 40m", "text-blue-600"], ["Milestones", "8", "text-rose-500"]].map(([label, value, color]) => <Card key={label}><CardContent><p className="text-xs font-bold text-slate-500">{label}</p><p className={`mt-1 text-2xl font-black ${color}`}>{value}</p><p className="text-xs text-slate-400">+5 this month</p></CardContent></Card>)}</section>
    <section className="rounded-2xl bg-slate-900 p-6 text-white sm:p-8"><p className="text-sm font-bold text-violet-300">Report narrative · {generatedAt}</p><h2 className="mt-2 max-w-3xl text-2xl font-black">You have a strong foundation. Building confidence in SQL now creates a practical bridge to Python and data cleaning.</h2><p className="mt-3 max-w-2xl text-sm leading-relaxed text-slate-300">You have kept momentum on your current path. Focus on one short SQL practice this week, then use an assessment when you are ready to verify your knowledge.</p></section>
...
export function NotebooksView() {
  ...
  return <div className="mx-auto max-w-6xl space-y-6"><div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-wider text-violet-600">Knowledge library</p><h1 className="mt-1 text-3xl font-black">Your notebooks</h1><p className="mt-2 text-slate-600">Associate notes with skills so you can bring them into a tutor chat.</p></div><Button className="bg-violet-600 hover:bg-violet-700" onClick={() => begin()}>+ New notebook</Button></div>
    <section className="rounded-2xl bg-slate-900 p-5 text-white"><p className="text-xs font-black uppercase tracking-wider text-violet-300">Pick up where you left off</p><p className="mt-2 font-black">SQL joins: key ideas and examples</p><p className="mt-1 text-sm text-slate-300">Linked to SQL fundamentals · continue building your study context.</p></section>

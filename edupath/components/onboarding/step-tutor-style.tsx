...
const toneOptions = [
  ...
  { value: "motivador", label: "Encouraging" },
];
...
export function StepTutorStyle({ values, errors, onChange }: StepTutorStyleProps) {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h3 className="text-lg font-semibold">Tutor style</h3>
        <p className="text-sm text-slate-600">Choose how you want explanations delivered.</p>
      </div>
      <SelectField
        label="Language"
        options={[
      ...
      <SelectField
        ...
        value={style.tone}
        error={errors.tone}
        onChange={(event) => update({ tone: event.target.value as typeof style.tone })}
      />
      <TextAreaField
